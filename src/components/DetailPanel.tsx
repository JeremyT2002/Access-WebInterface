...
export function DetailPanel({ schema, columns, row, onClose, onEdit }: Props) {
  ...
  return (
    <div className="w-80 shrink-0 border-l border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 h-full flex flex-col">
      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {onEdit && (
            <button
              onClick={onEdit}
              className="text-xs text-blue-600 hover:bg-blue-50 border border-blue-200 dark:border-blue-800 rounded-lg px-2 py-1"
            >
              ✎ Edit
            </button>
          )}
            ...
            <div key={c}>
              ...
              <div
                className={`text-sm mt-0.5 break-words whitespace-pre-wrap ${
                  v === null ? "text-slate-300 italic" : "text-slate-800 dark:text-slate-100"
                }`}
              >
                {v === null ? "NULL" : v === "" ? " " : v}
              </div>
